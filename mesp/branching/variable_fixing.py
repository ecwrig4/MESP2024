...
def varfix(V, Vsquare, E, n, d, s):
    ...
    for i in sone: #checks if discarding the i-th point improves solution
        ...
        indexT.remove(i)
        if len(S1) + len(S0) >= n -1:
            logging.warning("too many variables fixed")
...
def fix_variables(s: int, C: MespData) -> Tuple[bool, MespData, int, float]:
    ...
    if len(S1) + len(S0) >= C.n-1:
        logging.error("too many varfix")
        return False, C,s,time.time() - start_time
    if S1 == 0 and S0 == 0:
        ...
        if len(S1) > 0:
            ...
            if n_hat <= 0 or d_hat <= 0:
                logging.error("matrix after varfix is empty")
                return False, C, s, time.time()- start_time
